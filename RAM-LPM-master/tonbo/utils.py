...
def square_pad(img):
    if img.size[0] < img.size[1]:
        delta_0 = (img.size[1] - img.size[0]) // 2
        new_img = ImageOps.expand(img, (delta_0, 0, delta_0, 0))
    elif img.size[0] > img.size[1]:
        delta_1 = (img.size[0] - img.size[1]) // 2
        new_img = ImageOps.expand(img, (0, delta_1, 0, delta_1))
    else:
        new_img = img
    return new_img
